...
(function() {
        ...
        for (; 4 <= a.available();) {
            ...
            if (d === y.ECI) 0 === a.readBits(1) ? c.chunks.push({
                ...
                if (2 === g) {
                    g = e.readBits(7);
                    if (100 <=
                        g) throw Error("Invalid numeric value above 99");
                    e = Math.floor(g / 10);
                    g %= 10;
                    d.push(48 + e, 48 + g);
                ...
                d = [];
                f = "";
                for (g = e.readBits([9, 11, 13][b]); 2 <= g;) l = e.readBits(11), k = Math.floor(l / 45), l %= 45, d.push(B[k].charCodeAt(0), B[l].charCodeAt(0)),
                    f += B[k] + B[l], g -= 2;
            ...
            } else if (d === y.Byte) d = T(a, b), c.text += d.text, c.bytes.push(...d.bytes), c.chunks.push({
                type: t.Byte,
                bytes: d.bytes,
                text: d.text
            });
        ...
        for (var f = a.one; c.degree() >= d / 2;) {
            ...
            for (g = a.inverse(g); c.degree() >= b.degree() && !c.isZero();) {
                let d = c.degree() - b.degree(),
                    e = a.multiply(c.getCoefficient(c.degree()),
                        g);
    ...
    function Z(a, b, c) {
        ...
        for (var g of b.alignmentPatternCenters)
            for (var h of b.alignmentPatternCenters) 6 === g && 6 === h || 6 === g && h === e - 7 || g === e - 7 && 6 === h || f.setRegion(g - 2, h - 2, 5, 5, !0);
    ...
    function ha(a, b) {
        var c = fa({
            ...
            h = d.a11 * c.a21 + d.a21 * c.a22 + d.a31 *
            c.a23,
    ...
    function O(a, b, c) {
        try {
            ...
            return Math.sqrt(h.error * h.error + k.error * k.error + l.error * l.error + q.error * q.error) + (Math.pow(h.averageSize - p, 2) + Math.pow(k.averageSize - p, 2) + Math.pow(l.averageSize - p, 2) + Math.pow(q.averageSize - p, 2)) / p
        } catch (d) {
            return Infinity
        }
    }
    ...
    function H(a, b) {
        ...
        for (var d = Math.round(b.x); a.get(d,
                Math.round(b.y));) d++;
        c = (c + d) / 2;
    ...
    function ka(a) {
        ...
        for (let m = 0; m <= a.height; m++) {
            ...
            for (let b = -1; b <= a.width; b++) {
                ...
                if (g === l) f++;
                    ...
                    l = g;
                    var h = u(n) / 7;
                    h = Math.abs(n[0] - h) < h && Math.abs(n[1] - h) < h && Math.abs(n[2] - 3 * h) < 3 * h && Math.abs(n[3] - h) < h && Math.abs(n[4] - h) < h && !g;
                    var k = u(n.slice(-3)) / 3;
                    g = Math.abs(n[2] -
                    ...
                    if (h) {
                        ...
                        k = c.filter(b => d >= b.bottom.startX && d <= b.bottom.endX || a >= b.bottom.startX && d <= b.bottom.endX || d <= b.bottom.startX && a >= b.bottom.endX && 1.5 > n[2] / (b.bottom.endX - b.bottom.startX) && .5 < n[2] / (b.bottom.endX - b.bottom.startX));
                        0 < k.length ? k[0].bottom = h : c.push({
                            top: h,
                            bottom: h
                        })
                    }
                    if (g) {
                        ...
                        h = e.filter(b => c >= b.bottom.startX && c <= b.bottom.endX || a >= b.bottom.startX && c <= b.bottom.endX ||
                            c <= b.bottom.startX && a >= b.bottom.endX && 1.5 > n[2] / (b.bottom.endX - b.bottom.startX) && .5 < n[2] / (b.bottom.endX - b.bottom.startX));
                        0 < h.length ? h[0].bottom = g : e.push({
                            top: g,
                            bottom: g
                        })
                    }
        ...
        d.push(...e);
        b = b.filter(a => 2 <= a.bottom.y - a.top.y).map(b => {
            const c = (b.top.startX + b.top.endX + b.bottom.startX + b.bottom.endX) /
                4,
                d = (b.top.y + b.bottom.y + 1) / 2;
            if (a.get(Math.round(c), Math.round(d))) return b = [b.top.endX - b.top.startX, b.bottom.endX - b.bottom.startX, b.bottom.y - b.top.y + 1], b = u(b) / b.length, {
                score: O({
                    x: Math.round(c),
                    y: Math.round(d)
                }, [1, 1, 3, 1, 1], a),
    ...
    function P(a, b, c, d, e) {
        ...
        b = b.map(b => {
            const c = (b.top.startX + b.top.endX + b.bottom.startX + b.bottom.endX) / 4,
                d = (b.top.y + b.bottom.y + 1) / 2;
            if (a.get(Math.floor(c), Math.floor(d))) return u([b.top.endX -
                b.top.startX, b.bottom.endX - b.bottom.startX, b.bottom.y - b.top.y + 1
            ]), b = O({
                x: Math.floor(c),
                y: Math.floor(d)
            }, [1, 1, 1], a) + x({
        ...
        return {
            alignmentPattern: 15 <= c && b.length ? b[0] : l,
            dimension: f
        }
    }
        ...
        for (let e of b) {
            ...
            if (null == c) c = null;
                ...
                if (d) c = d;
                    ...
                    for (d = 0; d < c.width; d++)
                        for (let a = d + 1; a < c.height; a++) c.get(d, a) !== c.get(a, d) && (c.set(d, a, !c.get(d, a)), c.set(a, d, !c.get(a,
                            d)));
                    c = L(c)
                }
            }
    ...
    function I(a, b, c, d = {}) {
        ...
        if (g.useIntegerApproximation)
            for (var p = 0; p < c; p++)
                for (var m = 0; m < b; m++) {
                    var r = 4 * (p * b + m);
                    q.set(m, p, g.red * a[r] + g.green *
                        a[r + 1] + g.blue * a[r + 2] + 128 >> 8)
                } else
                    for (p = 0; p < c; p++)
                        for (m = 0; m < b; m++) r = 4 * (p * b + m), q.set(m, p, g.red * a[r] + g.green * a[r + 1] + g.blue * a[r + 2]);
        g = Math.ceil(b / 8);
        p = Math.ceil(c / 8);
        m = g * p;
        if (h) {
        ...
        for (m = 0; m < p; m++)
            for (r = 0; r < g; r++) {
                ...
                24 >= t - v && (u = v / 2, 0 < m && 0 < r && (t = (n.get(r, m - 1) + 2 * n.get(r - 1, m) + n.get(r - 1, m - 1)) / 4, v < t && (u =
                    t)));
                n.set(r, m, u)
            }
        ...
        for (b = 0; b < p; b++)
            for (a = 0; a < g; a++) {
                ...
                c = k / 25;
                for (h = 0; 8 > h; h++)
                    for (k = 0; 8 > k; k++) l = 8 * a + h, v = 8 * b + k, t = q.get(l, v), m.set(l, v, t <= c), f && r.set(l, v, !(t <= c))
            }
        f = f ? {
            binarized: m,
            inverted: r
    ...
    class V {
        ...
        readBits(a) {
            ...
            if (0 < this.bitOffset) {
                ...
                a -= c;
                this.bitOffset += c;
                8 === this.bitOffset && (this.bitOffset =
                    0, this.byteOffset++)
            }
            if (0 < a) {
